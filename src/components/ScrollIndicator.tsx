  ...
  return (
    <div className="fixed right-6 top-1/2 -translate-y-1/2 z-50 hidden md:flex flex-col items-center gap-4">
      {Array.from({ length: totalSections }).map((_, index) => (
        ...
        >
          {/* Label on hover */}
          <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-sm font-medium text-charcoal bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full shadow-sm border border-mustard/20 whitespace-nowrap">
            {sectionNames[index]}
          </span>
      ...
      {/* Progress line */}
      <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-0.5 bg-charcoal/10 -z-10 rounded-full">
        <div 
