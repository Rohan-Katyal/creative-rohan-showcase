...
interface ImmersiveVideosProps {
  progress: number;
}

const ImmersiveVideos = ({ progress }: ImmersiveVideosProps) => {
  const videos = [
    {
      id: "-ayhDIdxYvg",
  ...
  return (
    <div className="w-full h-full overflow-y-auto py-16 md:py-20 px-4 sm:px-6 lg:px-8 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10 md:mb-16">
          ...
          <div className="w-16 md:w-20 h-1 bg-gradient-to-r from-mustard to-mustard/70 mx-auto mb-6 md:mb-8 rounded-full" />
            ...
            <CarouselContent className="-ml-2 md:-ml-4">
              {videos.map((video, index) => (
                <CarouselItem key={video.id} className="pl-2 md:pl-4 basis-[70%] sm:basis-1/2 lg:basis-1/3">
                  <div 
                    className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group border border-mustard/20 h-full"
...
export default ImmersiveVideos;
